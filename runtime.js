...
defineCommand('voicemove', async (msg) => { const [_, fromId, toId] = msg.content.split(/\s+/); const from = msg.guild.channels.cache.get(fromId); const to = msg.guild.channels.cache.get(toId); if (!from||!to) return void msg.channel.send('Usage: +voicemove <fromId> <toId>'); const members = from.members; for (const m of members.values()) { try { await m.voice.setChannel(to); } catch {} } await msg.channel.send(`Tous les utilisateurs ont bien été déplacés vers ${to}`); });
defineCommand('voicekick', async (msg) => { const m = msg.mentions.members.first(); if (!m||!m.voice.channelId) return void msg.channel.send('Veuillez mentionner un utilisateur.'); const chId = m.voice.channelId; await m.voice.disconnect().catch(()=>{}); const ch = chId ? msg.guild.channels.cache.get(chId) : null; await msg.channel.send(`${m.user.username} a été déconnecté de la vocal ${ch ? ch.toString() : ''}`); });
defineCommand('bringall', async (msg) => { const to = msg.mentions.channels.first(); if (!to || to.type !== 2) return void msg.channel.send('Mentionne un salon vocal.'); const members = (await msg.guild.members.fetch()).filter(m=>m.voice.channelId); for (const m of members.values()) { try { await m.voice.setChannel(to); } catch {} } await msg.channel.send(`Tous les utilisateurs ont bien été déplacés vers ${to}`); });
